...
function renderPagination() {
  ...
  const buatItem = (label, halaman, disabled = false, aktif = false) => {
    const li = document.createElement('li');
    li.className = `page-item${disabled ? ' disabled' : ''}${aktif ? ' active' : ''}`;
  ...
  nav.appendChild(buatItem('«', state.halamanAktif - 1, state.halamanAktif === 1));

  for (let i = 1; i <= totalHlm; i++) {
    if (totalHlm > 7 && i > 3 && i < totalHlm - 1 && Math.abs(i - state.halamanAktif) > 1) {
      if (i === 4) nav.appendChild(buatItem('…', null, true));
      continue;
    }
    nav.appendChild(buatItem(i, i, false, i === state.halamanAktif));
  }

  nav.appendChild(buatItem('»', state.halamanAktif + 1, state.halamanAktif === totalHlm));
}
